# Provider Key Re-Sync — Standalone Recipe (orchestrator scope)

> Este arquivo é compartilhado conceitualmente com `instance-number/references/provider-key-resync.md`
> mas mora no umbrella porque é uma operação **de sistema** (afeta todos os perfis),
> não de provisionamento de um cliente específico.

---

## Quando usar

- Admin rotacionou `OLLAMA_API_KEY` no `~/.hermes/.env`
- Admin mudou de provedor (Ollama → OpenRouter, etc.)
- Admin adicionou nova chave
- Após incidente onde creds foram comprometidos

## Por que `config.yaml` e `.env` se comportam diferente
...
`config.yaml` é symlink porque provider/modelo é o mesmo pra todos os perfis neste ambiente. As chaves de API são cópia porque (a) gateway lê `$HERMES_HOME/.env` no startup e (b) ter cada perfil com sua própria cópia isola falhas de provisionamento.
...
for profile_dir in "$PROFILES_DIR"/*/; do
  ...
  for key in "${PROVIDER_KEYS[@]}"; do
    if ! grep -q "^$key=" "$GLOBAL_ENV"; then continue; fi
...
echo ""
echo "Atualizadas: $UPDATED chave(s) | Puladas: $SKIPPED"
```
...
Use `edit-instance` SKILL.md, Phase 6.
...
Gateway não tem hot-reload de .env — lê uma vez no startup e cacheia. Por isso o re-sync precisa matar e relançar o processo, não basta editar o arquivo. Alternativas futuras (não implementadas):
- Gateway ler .env a cada mensagem
- Signal handler que recarrega config on SIGUSR1
- `config.yaml` watched via inotify

Por ora: restart. ~5s downtime, correto.